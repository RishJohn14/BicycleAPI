...
public class APIAgentLauncher extends JPSAgent
{
    ...
    private static final String AGENT_ERROR_MSG = "The BicycleAPI input agent could not be constructed!";
    ...
    private static final String CONNECTOR_ERROR_MSG = "Could not construct the Bicycle API connector needed to interact with the API!";
    ...
    public static JSONObject initializeAgent(String []args)
    {
        ...
        JSONObject bicycleReadings;

        try
        {
            bicycleReadings = connector.getReadings();
        }
        catch(Exception e)
        {
        ...
        Log.info(String.format("Retrieved %d bicycle readings", bicycleReadings.length()));
        jsonMessage.accumulate("Result","Retrieved"+bicycleReadings.getJSONArray("value").length()+" bicycle parking readings");
        ...
        {
            Log.info("No new bicycle parking data recorded");
        ...
        try
        {
            ...
            Log.info("All Data IRIs within Bicycle Parking Readings successfully instantiated");
